...
export default function CommandTerminal() {
  ...
  useEffect(() => {
    const channel = supabase
      .channel('kingmaker_conversations_changes')
      .on(
        'postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'kingmaker_conversations' },
        (payload) => {
          const conv = payload.new as KingmakerConversation;
          // Add user message
          setMessages((prev) => [
            ...prev,
            {
  ...
  const loadRecentConversations = async () => {
    try {
      ...
      if (data && data.length > 0) {
        const msgs: Message[] = [];
        data.forEach((conv: KingmakerConversation) => {
          msgs.push({
            id: conv.id,
            message: conv.user_message,
            timestamp: conv.created_at,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Call Supabase edge function to orchestrate the task
      ...
      const response: Message = {
        ...
        message: data?.message || data?.result || `Task completed: "${command}"`,
        timestamp: new Date().toISOString(),
        isUser: false,
      };
